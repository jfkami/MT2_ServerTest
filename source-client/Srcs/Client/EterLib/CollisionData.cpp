		...
		case COLLISION_TYPE_BOX:
			{
				...
				float fHalfWidth = c_pCollisionData->fDimensions[0] / 2.0f;
				float fHalfHeight = c_pCollisionData->fDimensions[2] / 2.0f;
				float fHalfLength = c_pCollisionData->fDimensions[1];
				TAABBData& AABBData = paci->GetAttribute();
				AABBData.v3Min.x = v3Pos.x - fHalfWidth;
				AABBData.v3Min.y = v3Pos.y - fHalfHeight;
				AABBData.v3Min.z = v3Pos.z - fHalfLength;
				AABBData.v3Max.x = v3Pos.x + fHalfWidth;
				AABBData.v3Max.y = v3Pos.y + fHalfHeight;
				AABBData.v3Max.z = v3Pos.z + fHalfLength;
...
{
	auto val = (s.v3Position-m_attribute.v3Position);
	if (D3DXVec3LengthSq(&val)>=(s.fRadius+m_attribute.fRadius)*(m_attribute.fRadius+s.fRadius))
		return D3DXVECTOR3(0.0f,0.0f,0.0f);
	D3DXVECTOR3 c;
	auto val1 = (s.v3Position-s.v3LastPosition);
	auto val2 = D3DXVECTOR3(0.0f,0.0f,1.0f);
	D3DXVec3Cross(&c, &val1, &val2);

	auto val3 = (s.v3Position-m_attribute.v3Position);
	float sum = - D3DXVec3Dot(&c,&val3);
	auto val4 = (s.v3Position-m_attribute.v3Position);
	float mul = (s.fRadius+m_attribute.fRadius)*(s.fRadius+m_attribute.fRadius)-D3DXVec3LengthSq(&val4);
		...
		if (D3DXVec3Dot(&v3QuadPosition1, &m_attribute.v3InsideVector[0]) > - s.fRadius/*0.0f*/)
			if (D3DXVec3Dot(&v3QuadPosition1, &m_attribute.v3InsideVector[1]) > -s.fRadius/*0.0f*/)
				if (D3DXVec3Dot(&v3QuadPosition2, &m_attribute.v3InsideVector[2]) > - s.fRadius/*0.0f*/)
					...
					{
						auto val1 = (s.v3Position - m_attribute.v3Position);
						auto val2 = (s.v3LastPosition - m_attribute.v3Position);
						if (fabs(D3DXVec3Dot(&val1, &m_attribute.v3Normal)) <
							fabs(D3DXVec3Dot(&val2, &m_attribute.v3Normal)))
							return true;
					}
	}
...
D3DXVECTOR3 CPlaneCollisionInstance::OnGetCollisionMovementAdjust(const CDynamicSphereInstance & s) const
{
	...
	if (d>=-0.0001 && d<=0.0001)
		return D3DXVECTOR3(0.0f,0.0f,0.0f);
	auto val = (s.v3Position-m_attribute.v3Position);
	float t= - D3DXVec3Dot(&m_attribute.v3Normal, &val)/d;
...
bool CCylinderCollisionInstance::CollideCylinderVSDynamicSphere(const TCylinderData & c_rattribute, const CDynamicSphereInstance & s) const
{
	...
	auto val = (oa-ob);
	return (D3DXVec3LengthSq(&val)<=(c_rattribute.fRadius+s.fRadius)*(c_rattribute.fRadius+s.fRadius));
}
...
D3DXVECTOR3 CCylinderCollisionInstance::OnGetCollisionMovementAdjust(const CDynamicSphereInstance & s) const
{
	...
	auto val4 = (s.v3Position-v3Position);
	if (D3DXVec3LengthSq(&val4)>=(s.fRadius+m_attribute.fRadius)*(m_attribute.fRadius+s.fRadius))
		return D3DXVECTOR3(0.0f,0.0f,0.0f);
	D3DXVECTOR3 c;
	D3DXVECTOR3 advance = s.v3Position - s.v3LastPosition;
	advance.z = 0;
	auto val3 = D3DXVECTOR3(0.0f,0.0f,1.0f);
	D3DXVec3Cross(&c, &advance, &val3);

	auto val2 = (s.v3Position-v3Position);
	float sum = - D3DXVec3Dot(&c,&val2);
	auto val = (s.v3Position-v3Position);
	float mul = (s.fRadius+m_attribute.fRadius)*(s.fRadius+m_attribute.fRadius)-D3DXVec3LengthSq(&val);
...
D3DXVECTOR3 CAABBCollisionInstance::OnGetCollisionMovementAdjust(const CDynamicSphereInstance & s) const
{
	...
	*/
	...
	auto val = (v3Temp - s.v3Position);
	if(D3DXVec3LengthSq(&val) < s.fRadius * s.fRadius)
