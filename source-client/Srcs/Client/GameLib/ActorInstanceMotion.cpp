...
#define ENABLE_WALK_RUN_ANI_ALTERNATIVE
void CActorInstance::SetLoopMotion(DWORD dwMotion, float fBlendTime, float fSpeedRatio)
{
	if (!m_pkCurRaceData)
	{
		if (dwMotion != CRaceMotionData::NAME_WALK)
			Tracenf("CActorInstance::SetLoopMotion(dwMotion=%d, fBlendTime=%f, fSpeedRatio=%f) for race %u",
			dwMotion, fBlendTime, fSpeedRatio, m_pkCurRaceData->GetRace()); // @warme003 race
		return;
	}
	...
	{
		#ifdef ENABLE_WALK_RUN_ANI_ALTERNATIVE
		if (dwMotion != CRaceMotionData::NAME_WALK)
		#endif
		{
			Tracenf("CActorInstance::SetLoopMotion(dwMotion=%d, fBlendTime=%f, fSpeedRatio=%f) - GetMotionKey(m_wcurMotionMode=%d, dwMotion=%d, &MotionKey) for race %u ERROR",
			dwMotion, fBlendTime, fSpeedRatio, m_wcurMotionMode, dwMotion, m_pkCurRaceData->GetRace()); // @warme003 race
			return;
		}
		#ifdef ENABLE_WALK_RUN_ANI_ALTERNATIVE
		else if (m_pkCurRaceData->GetMotionKey(CRaceMotionData::NAME_RUN, dwMotion, &dwMotionKey))
		{
			m_wcurMotionMode = CRaceMotionData::NAME_RUN;
		}
		else
			return;
		#endif
