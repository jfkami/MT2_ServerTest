...
int CPythonEventManager::RegisterEventSet(const char * c_szFileName)
{
	...
	const auto scriptGroup = pEventSet->ScriptGroup.Create(strEventString);
	if (-1 == scriptGroup)
	{
	...
	pEventSet->iTotalLineCount = scriptGroup;
#endif
...
int CPythonEventManager::RegisterEventSetFromString(const string& strScript)
{
	...
	const auto scriptGroup = pEventSet->ScriptGroup.Create(strScript);
	if (-1 == scriptGroup)
	{
		__ClearEventSetp(pEventSet);
		return -1;
...
void CPythonEventManager::SetFontColor(int iIndex, DWORD dwColor)
{
	if (!CheckEventSetIndex(iIndex))
		return;
	...
	pEventSet->CurrentColor = dwColor;
	if (pEventSet->pCurrentTextLine)
		pEventSet->pCurrentTextLine->SetColor(pEventSet->CurrentColor.r, pEventSet->CurrentColor.g, pEventSet->CurrentColor.b);
}
#endif
...
{
	auto iCount = 0;
#ifdef USE_NEW_EVENT_TEXT_AUTO_Y
	auto iPrevHeight = 0;
#endif
	...
	for (auto itor = pEventSet->ScriptTextLineList.begin(); itor != pEventSet->ScriptTextLineList.end(); ++itor, ++iCount)
	{
		...
		CGraphicTextInstance * pInstance = rkLine.pInstance;
#ifdef USE_NEW_EVENT_TEXT_AUTO_Y
		if (pEventSet->iVisibleStartLine > 0) // don't check not-skipped events
		{
			if ((iCount >= pEventSet->iVisibleStartLine)
				&& (iCount < pEventSet->iVisibleStartLine + pEventSet->iVisibleLineCount)) {
				rkLine.iyLocal = iPrevHeight;
				iPrevHeight += c_fLine_Temp;
			}
		}
#endif
