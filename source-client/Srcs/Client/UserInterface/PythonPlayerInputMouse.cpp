...
void CPythonPlayer::NEW_RefreshMouseWalkingDirection()
{
	...
	switch (m_eReservedMode)
	{
		case MODE_CLICK_ITEM:
		{
			...
			{
				auto pickDistance = 20.0f;
				if (pkInstMain->IsMountingHorse() || pkInstMain->IsNewMount()) // @fixme037
					pickDistance = 110.0f;

				if (pkInstMain->NEW_GetDistanceFromDestPixelPosition(kPPosPickedItem) < pickDistance)
				{
