...
bool CEterPack::Get(CMappedFile& out_file, const char * filename, LPCVOID * data)
{
	...
	else if (COMPRESSED_TYPE_HYBRIDCRYPT == index->compressed_type || COMPRESSED_TYPE_HYBRIDCRYPT_WITHSDB == index->compressed_type)
	{
		...
		if( !m_pCSHybridCryptPolicy->DecryptMemory(filename, static_cast<const BYTE*>(*data), index->data_size, *zObj) )
		{
			delete zObj; // @fixme035
			return false;
		}
		...
		if( COMPRESSED_TYPE_HYBRIDCRYPT_WITHSDB == index->compressed_type)
		{
			...
			if( !m_pCSHybridCryptPolicy->GetSupplementaryDataBlock(filename, pSDBData, iSDBSize) )
			{
				delete zObj; // @fixme035
				return false;
			}
...
bool CEterPack::Extract()
{
		...
		++i)
	{
		...
		{
			if (!__Decrypt_Panama(index->filename, (const BYTE *) data + index->data_position, index->data_size, zObj))
			{
				printf("cannot decompress %s type %d\n", index->filename, index->compressed_type);
				return false;
			}
		...
		else if (COMPRESSED_TYPE_HYBRIDCRYPT == index->compressed_type || COMPRESSED_TYPE_HYBRIDCRYPT_WITHSDB == index->compressed_type)
		{
			...
			VM_START
#endif
			if (!m_pCSHybridCryptPolicy->DecryptMemory(index->filename, (const BYTE*)data + index->data_position, index->data_size, zObj))
			{
				printf("cannot decompress %s type %d\n", index->filename, index->compressed_type);
				return false;
			}
			...
			if( COMPRESSED_TYPE_HYBRIDCRYPT_WITHSDB == index->compressed_type)
			{
				...
				if (!m_pCSHybridCryptPolicy->GetSupplementaryDataBlock(index->filename, pSDBData, iSDBSize))
				{
					printf("cannot decompress %s type %d\n", index->filename, index->compressed_type);
					return false;
				}
...
bool CEterPack::Put(const char * filename, const char * sourceFilename, BYTE packType, const std::string& strRelateMapName )
{
	...
	if( packType == COMPRESSED_TYPE_HYBRIDCRYPT || packType == COMPRESSED_TYPE_HYBRIDCRYPT_WITHSDB )
	{
		...
		VM_START
#endif
		m_pCSHybridCryptPolicy->GenerateCryptKey( filename );

		if( packType == COMPRESSED_TYPE_HYBRIDCRYPT_WITHSDB )
		{
			if( !m_pCSHybridCryptPolicy->GenerateSupplementaryDataBlock( filename, strRelateMapName, (const BYTE*)data, mapFile.Size(), pMappedData, iMappedDataSize ))
			{
				return false;
			}
		}
...
bool CEterPack::Put(const char * filename, LPCVOID data, long len, BYTE packType)
{
	...
	else if (packType == COMPRESSED_TYPE_HYBRIDCRYPT || packType == COMPRESSED_TYPE_HYBRIDCRYPT_WITHSDB )
	{
		...
		if( !m_pCSHybridCryptPolicy->EncryptMemory( filename, (const BYTE *)data, len, zObj ) )
		{
			return false;
		}
