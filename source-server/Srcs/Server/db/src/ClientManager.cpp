...
void CClientManager::QUERY_BOOT(CPeer* peer, TPacketGDBoot * p)
{
	...
	peer->Encode(dwPacketSize);
	peer->Encode(bPacketVersion);
	...
	peer->EncodeWORD(1);
	peer->Encode(itemRange);
	peer->Encode(itemRangeSpare);
	...
	if (num_monarch_candidacy)
		peer->Encode(rVecMonarchCandidacy);
	//END_MONARCE
...
void CClientManager::SendPartyOnSetup(CPeer* pkPeer)
{
	...
	for (itertype(pm) it_party = pm.begin(); it_party != pm.end(); ++it_party)
	{
		...
		for (itertype(it_party->second) it_member = it_party->second.begin(); it_member != it_party->second.end(); ++it_member)
		{
			...
			pkPeer->Encode(it_party->first);
			pkPeer->Encode(it_member->first);
			pkPeer->Encode(it_member->second.bRole);

			pkPeer->EncodeHeader(HEADER_DG_PARTY_SET_MEMBER_LEVEL, 0, sizeof(TPacketPartySetMemberLevel));
			pkPeer->Encode(it_party->first);
			pkPeer->Encode(it_member->first);
			pkPeer->Encode(it_member->second.bLevel);
		}
	}
}
...
void CClientManager::RESULT_SAFEBOX_LOAD(CPeer * pkPeer, SQLMsg * msg)
{
	...
	else
	{
			...
			if (!bEscape)
			{
				...
				while (it != pSet->end())
				{
					...
					if (pItemTable->bType == ITEM_UNIQUE)
					{
						...
						const auto lValue0 = pItemTable->alValues[ITEM_SOCKET_REMAIN_SEC];
						const auto lValue2 = pItemTable->alValues[ITEM_SOCKET_UNIQUE_REMAIN_TIME];
						const auto tNow = CClientManager::instance().GetCurrentTime();
...

					{
						...
						ItemAwardManager::instance().CheckItemAttributes(*pItemAward, *pItemTable, m_vec_itemAttrTable, m_vec_itemRareTable);
						#endif

						// START_OF_AUTO_QUERY
						char szColumns[QUERY_MAX_LEN], szValues[QUERY_MAX_LEN];

						auto iLen = snprintf(szColumns, sizeof(szColumns), "id, owner_id, `window`, pos, vnum, count");
						auto iValueLen = snprintf(szValues, sizeof(szValues), "%u, %u, '%s', %d, %u, %u", GainItemID(), pi->account_id, (pi->ip[0] == 0) ? "SAFEBOX" : "MALL", iPos, pItemAward->dwVnum, pItemAward->dwCount);
		...
		pkPeer->Encode(*pi->pSafebox);

		if (!s_items.empty())
			pkPeer->Encode(s_items);
...
void CClientManager::QUERY_EMPIRE_SELECT(CPeer * pkPeer, DWORD dwHandle, TEmpireSelectPacket * p)
{
	...
	sys_log(0, "EmpireSelect: %s", szQuery);
	{
		...
		if (pRes->uiNumRows)
		{
			...
			DWORD pids[PLAYER_PER_ACCOUNT]{}; // @fixme310
			...
			for (int i = 0; i < PLAYER_PER_ACCOUNT; ++i) // @fixme310
			{
...
void CClientManager::QUERY_SETUP(CPeer * peer, DWORD dwHandle, const char * c_pData)
{
	...
	kMapLocations.channel = peer->GetChannel();
	#endif
	...
	for (auto & tmp : m_peerList) {
		...
		TMapLocation kMapLocation2{};
		kMapLocation2.channel = tmp->GetChannel();
		strlcpy(kMapLocation2.szHost, tmp->GetPublicIP(), sizeof(kMapLocation2.szHost));
		kMapLocation2.wPort = tmp->GetListenPort();
		thecore_memcpy(kMapLocation2.alMaps, tmp->GetMaps(), sizeof(kMapLocation2.alMaps));
		vec_kMapLocations.emplace_back(kMapLocation2);
	...
	for (DWORD c = 0; c < p->dwLoginCount; ++c, ++pck)
	{
		...
		if (InsertLogonAccount(pck->szLogin, peer->GetHandle(), pck->szHost))
		{
			...
			if (m_pkAuthPeer)
			{
				...
				vec_repair.emplace_back(pck_repair);
			}
		}
		else
	...
	if (m_pkAuthPeer && !vec_repair.empty())
	{
		...
		m_pkAuthPeer->Encode(vec_repair);
	}
...
void CClientManager::QUERY_RELOAD_PROTO()
{
	...
	for (TPeerList::iterator i = m_peerList.begin(); i != m_peerList.end(); ++i)
	{
		...
		tmp->Encode(m_vec_skillTable);

		tmp->EncodeWORD(m_vec_banwordTable.size());
		tmp->Encode(m_vec_banwordTable);

		tmp->EncodeWORD(m_vec_itemTable.size());
		tmp->Encode(m_vec_itemTable);

		tmp->EncodeWORD(m_vec_mobTable.size());
		tmp->Encode(m_vec_mobTable);
	}
}
...
void CClientManager::InsertLoginData(CLoginData * pkLD)
{
	...
	m_map_pkLoginDataByLogin.emplace(szLogin, pkLD);
	m_map_pkLoginDataByAID.emplace(pkLD->GetAccountRef().id, pkLD);
}
...
void CClientManager::CreateObject(TPacketGDCreateObject * p)
{
	...
	m_map_pkObjectTable.emplace(pkObj->dwID, pkObj);
}
...
void CClientManager::ProcessPackets(CPeer * peer)
{
	...
	while (peer->PeekPacket(i, header, dwHandle, dwLength, &data))
	{
		...
		if (g_test_server)
		{
			if (header != HEADER_GD_PLAYER_COUNT)
...
void CClientManager::RequestChannelStatus(CPeer* peer, DWORD dwHandle)
{
	...
	for (TChannelStatusMap::iterator it = m_mChannelStatus.begin(); it != m_mChannelStatus.end(); it++) {
		peer->Encode(it->first);
		peer->Encode(it->second);
	}
}
