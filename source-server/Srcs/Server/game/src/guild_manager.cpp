...
CGuild* CGuildManager::TouchGuild(DWORD guild_id)
{
	...
	{
		m_mapGuild.emplace(guild_id, M2_NEW CGuild(guild_id));
...
void CGuildManager::Kill(LPCHARACTER killer, LPCHARACTER victim)
{
	...
	if (killer->IsObserverMode() || victim->IsObserverMode()) // @fixme193
		return;
...
void CGuildManager::ReserveWarBet(TPacketGDGuildWarBet * p)
{
	...
	it->second->mapBet.emplace(p->szLogin, std::make_pair(p->dwGuild, p->dwGold));
}
...
void CGuildManager::ChangeMaster(DWORD dwGID)
{
	...
	DBManager::instance().FuncQuery(msl::bind1st(std::mem_fn(&CGuild::SendGuildDataUpdateToAllMember), iter->second),
			"SELECT 1");
}
