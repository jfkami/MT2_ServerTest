...
namespace quest
{
	...
	void PC::SaveFlag(const string & name, int value)
	{
		...
		if (it == m_FlagSaveMap.end())
			m_FlagSaveMap.emplace(name, value);
		else if (it->second != value)
			it->second = value;
	}
	...
	void PC::AddQuestStateChange(const string& quest_name, int prev_state, int next_state)
	{
		...
		m_QuestStateChange.emplace_back(TQuestStateChangeInfo(dwQuestIndex, prev_state, next_state));
	}
	...
	void PC::SendQuestInfoPakcet()
	{
		...
		CQuestManager::instance().GetCurrentCharacterPtr()->GetDesc()->BufferedPacket(qi); // @fixme302
	...
	void PC::DoQuestStateChange()
	{
		...
		for (DWORD i=0; i<vecQuestStateChange.size(); ++i)
		{
			...
			if (it == quest_end())
			{
				...
				m_QuestInfo.emplace(dwQuestIdx, qs);
