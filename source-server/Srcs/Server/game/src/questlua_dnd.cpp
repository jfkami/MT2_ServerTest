...
namespace quest
{
	...
	static ALUA(item_get_cell)
	{
		CQuestManager& q = CQuestManager::instance();

		if (q.GetCurrentDNDItem())
		{
			lua_pushnumber(L, q.GetCurrentDNDItem()->GetCell());
		}
		else
			lua_pushnumber(L, 0);
		return 1;
	...
	static ALUA(item_select_cell)
	{
		...
		CQuestManager::instance().SetCurrentDNDItem(item);
		lua_pushboolean(L, 1);

		return 1;
	}

	static ALUA(item_select)
	{
		lua_pushboolean(L, 0);
		if (!lua_isnumber(L, 1))
		{
	...
	static ALUA(item_get_id)
	{
		CQuestManager& q = CQuestManager::instance();

		if (q.GetCurrentDNDItem())
		{
			lua_pushnumber(L, q.GetCurrentDNDItem()->GetID());
		}
		else
			lua_pushnumber(L, 0);
		return 1;
	...
	static ALUA(item_get_refine_vnum)
	{
		CQuestManager& q = CQuestManager::instance();
		LPITEM item = q.GetCurrentDNDItem();
	...
	static ALUA(item_next_refine_vnum)
	{
		DWORD vnum = 0;
		if (lua_isnumber(L, 1))
	...
	static ALUA(item_can_over9refine)
	{
		LPITEM item = CQuestManager::instance().GetCurrentDNDItem();
	...
	static ALUA(item_change_to_over9)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		LPITEM item = CQuestManager::instance().GetCurrentDNDItem();
	...
	static ALUA(item_get_over9_material_vnum)
	{
	...
	static ALUA(item_get_level_limit)
	{
		CQuestManager& q = CQuestManager::instance();

		if (q.GetCurrentDNDItem())
		{
			if (q.GetCurrentDNDItem()->GetType() != ITEM_WEAPON && q.GetCurrentDNDItem()->GetType() != ITEM_ARMOR)
			{
				return 0;
			}
	...
	static ALUA(item_start_realtime_expire)
	{
		CQuestManager& q = CQuestManager::instance();
		LPITEM pItem = q.GetCurrentDNDItem();
	...
	static ALUA(item_copy_and_give_before_remove)
	{
		lua_pushboolean(L, 0);
		if (!lua_isnumber(L, 1))
			return 1;
	...
	static ALUA(item_get_wearflag0)
	{
		CQuestManager& q = CQuestManager::instance();
		LPITEM item = q.GetCurrentDNDItem();
	...
	static ALUA(item_has_wearflag0)
	{
		CQuestManager& q = CQuestManager::instance();
		LPITEM item = q.GetCurrentDNDItem();
	...
	static ALUA(item_get_antiflag0)
	{
		CQuestManager& q = CQuestManager::instance();
		LPITEM item = q.GetCurrentDNDItem();
	...
	static ALUA(item_has_antiflag0)
	{
		CQuestManager& q = CQuestManager::instance();
		LPITEM item = q.GetCurrentDNDItem();
	...
	static ALUA(item_get_immuneflag0)
	{
		CQuestManager& q = CQuestManager::instance();
		LPITEM item = q.GetCurrentDNDItem();
	...
	static ALUA(item_has_immuneflag0)
	{
		CQuestManager& q = CQuestManager::instance();
		LPITEM item = q.GetCurrentDNDItem();
...
#define NS_ITEM_GETMODE0(x)	\
	...
	static ALUA(item_is_available0)
	{
		CQuestManager& q = CQuestManager::instance();
		LPITEM item = q.GetCurrentDNDItem();
	...
	void RegisterDNDFunctionTable()
	{
		...
		CQuestManager::instance().AddLuaFunctionTable("dnd", item_functions);
	}
}
#endif
