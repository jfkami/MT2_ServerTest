...
void CGuild::LoadGuildData(SQLMsg* pmsg)
{
	...
	#ifdef ENABLE_GUILD_TOKEN_AUTH
	str_to_number(m_data.token, row[12]);
	#endif
...
void CGuild::AddComment(LPCHARACTER ch, const std::string& str)
{
	...
	if (!PulseManager::Instance().IncreaseClock(ch->GetPlayerID(), ePulse::GuildComment, std::chrono::milliseconds(1500))) {
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Remaining time: %.2f"), PULSEMANAGER_CLOCK_TO_SEC2(ch->GetPlayerID(), ePulse::GuildComment));
		return;
	}
#endif
...
void CGuild::RefreshCommentForce(DWORD player_id)
{
	...
	for (uint i = 0; i < pmsg->Get()->uiNumRows; i++)
	{
		...
		d->BufferedPacket(id);
		d->BufferedPacket(szName);

		if (i == pmsg->Get()->uiNumRows - 1)
			d->Packet(szContent);
		else
			d->BufferedPacket(szContent);
	}
}
...
void CGuild::Chat(const char* c_pszText)
{
	...
	P2P_MANAGER::instance().Send(p1);
	P2P_MANAGER::instance().Send(p2);
}
...
void CGuild::BroadcastMemberCountBonus()
{
	...
	P2P_MANAGER::instance().Send(p1);
	P2P_MANAGER::instance().Send(m_iMemberCountBonus);
}
