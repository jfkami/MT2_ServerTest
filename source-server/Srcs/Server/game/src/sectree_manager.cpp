...
void SECTREE_MAP::Build()
{
	...
	while (it != map_.end())
	{
		...
		tree->m_neighbor_list.emplace_back(tree);
...
int SECTREE_MANAGER::Build(const char * c_pszListFileName, const char* c_pszMapBasePath)
{
	...
	msl::file_ptr f(c_pszListFileName);
	if (!f.is_open())
		return 0;

	std::unordered_set<DWORD> loadedMaps; // @fixme308
	auto lines = msl::string_split(f.string_read(), '\n');
	for (auto & line : lines)
	{
		...
		auto tokens = msl::string_split_any(line.data(), " \t");
		if (tokens.size() < 2) // mapId mapName
		{
			if (!line.empty() && line.size() > 2)
				sys_err("%s: can't load the line '%s'", c_pszListFileName, line.c_str());
			continue;
		}

		auto iIndex = std::stoi(tokens[0]);
		auto szMapName = tokens[1].c_str();

		if (loadedMaps.find(iIndex) != loadedMaps.end()) // @fixme308
		{
			sys_err("%s: map %d already loaded", c_pszListFileName, iIndex);
			continue;
		}
		loadedMaps.emplace(iIndex);

		auto filename = fmt::format("{}/{}/Setting.txt", c_pszMapBasePath, szMapName);

		TMapSetting setting;
		setting.iIndex = iIndex;

		if (!LoadSettingFile(iIndex, filename.c_str(), setting))
		{
			sys_err("can't load file %s in LoadSettingFile", filename.c_str());
			return 0;
		}

		filename = fmt::format("{}/{}/Town.txt", c_pszMapBasePath, szMapName);

		if (!LoadMapRegion(filename.c_str(), setting, szMapName))
		{
			sys_err("can't load file %s in LoadMapRegion", filename.c_str());
			return 0;
		}
		...
		if (map_allow_find(iIndex))
		{
			...
			filename = fmt::format("{}/{}/server_attr", c_pszMapBasePath, szMapName);
			LoadAttribute(pkMapSectree, filename.c_str(), setting);

			filename = fmt::format("{}/{}/regen.txt", c_pszMapBasePath, szMapName);
			regen_load(filename.c_str(), setting.iIndex, setting.iBaseX, setting.iBaseY);

			filename = fmt::format("{}/{}/npc.txt", c_pszMapBasePath, szMapName);
			regen_load(filename.c_str(), setting.iIndex, setting.iBaseX, setting.iBaseY);

			filename = fmt::format("{}/{}/boss.txt", c_pszMapBasePath, szMapName);
			regen_load(filename.c_str(), setting.iIndex, setting.iBaseX, setting.iBaseY);

			filename = fmt::format("{}/{}/stone.txt", c_pszMapBasePath, szMapName);
			regen_load(filename.c_str(), setting.iIndex, setting.iBaseX, setting.iBaseY);

			filename = fmt::format("{}/{}/dungeon.txt", c_pszMapBasePath, szMapName);
			LoadDungeon(iIndex, filename.c_str());
...
std::pair<long, long> SECTREE_MAP::CoordinateToLocal(long x, long y)
{
	auto nx = x - m_setting.iBaseX;
	auto ny = y - m_setting.iBaseY;
	return { nx / 100, ny / 100 };
}

std::pair<long, long> SECTREE_MAP::CoordinateToGlobal(long x, long y)
{
	auto nx = x * 100 + m_setting.iBaseX;
	auto ny = y * 100 + m_setting.iBaseY;
	return { nx, ny };
}
