...
{
	m_map_pkObject.emplace(pkObj->GetID(), pkObj);
	m_map_pkObjectByVID.emplace(pkObj->GetVID(), pkObj);
...
bool CManager::LoadObjectProto(const TObjectProto * pProto, int size) // from DB
{
	...
	for (int i = 0; i < size; ++i)
	{
		...
		m_map_pkObjectProto.emplace(r.dwVnum, &m_vec_kObjectProto[i]);
	}
...
bool CManager::LoadLand(TLand * pTable) // from DB
{
	...
	m_map_pkLand.emplace(pkLand->GetID(), pkLand);
...
{
	if (!map_allow_find(pTable->lMapIndex)) // @fixme315
		return false;
	...
	m_map_pkObjByVID.emplace(dwVID, pkObj);
	m_map_pkObjByID.emplace(pTable->dwID, pkObj);
