...
#define ENABLE_WHEN_PARENTHESIS
...
{
	function_defs.emplace(fname);
}

void RegisterUsedFunction(const string& fname)
{
	function_calls.emplace(fname);
}
...
void parse(char * filename)
{
			...
			case ST_STATE_NAME:
				{
					...
					if (t.token == TK_NAME || t.token == TK_STRING)
					{
						...
						define_state_name_set.emplace(current_state_name);
			...
			case ST_WHEN_NAME:
				{
					...
					if (t.token == TK_NAME || t.token == TK_STRING || t.token == TK_NUMBER)
					{
...

						{
...
#ifdef ENABLE_WHEN_PARENTHESIS
							//Accept functions as valid arguments
							constexpr auto TK_OPEN_PARENTHESIS = '(';
							constexpr auto TK_CLOSE_PARENTHESIS = ')';
							...
							constexpr auto TK_OPEN_PARENTHESIS2 = '[';
							constexpr auto TK_CLOSE_PARENTHESIS2 = ']';
							...
							if (lexstate.lookahead.token == TK_OPEN_PARENTHESIS2)
							{
								...
								{
									if (lexstate.lookahead.token == TK_OPEN_PARENTHESIS2) //allow function calls inside as well
										depth++;
	...
	if (!define_state_name_set.empty())
	{
		...
		state_crc["start"] = 0;
		for (auto it = define_state_name_set.begin(); it != define_state_name_set.end(); ++it)
		{
	...
	{
		for (auto it = state_arg_script_map.begin(); it != state_arg_script_map.end(); ++it)
		{
			string second_name;
			string path;
