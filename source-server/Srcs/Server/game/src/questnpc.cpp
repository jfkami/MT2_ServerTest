...
namespace quest
{
	...
	bool NPC::OnBoot(PC& pc)
	{
		return HandleReceiveAllEvent(pc, QUEST_BOOT_EVENT);
	}
#endif
	...
	bool NPC::OnDND(PC & pc, bool bReceiveAll)
	{
		if (bReceiveAll)
			...
			return HandleEvent(pc, QUEST_DND_EVENT);
	}
#endif
	...
	struct FuncMissHandleEvent
	{
		...
		void operator()(PC::QuestInfoIterator& itPCQuest, NPC::QuestMapType::iterator& itQuestMap)
		{
			...
			{
				size++;
				vdwNewStartQuestIndices.emplace_back(dwQuestIndex);
			}
		}
	};
	...
	struct FuncMatchHandleEvent
	{
		...
		void operator()(PC::QuestInfoIterator& itPCQuest, NPC::QuestMapType::iterator& itQuestMap)
		{
			...
			if ((itQuestScript = itQuestMap->second.find(iState)) != itQuestMap->second.end())
			{
				bMatched = true;
				size++;
				vdwQuesIndices.emplace_back(itQuestMap->first);
				viPCStates.emplace_back(iState);
				vcodes.emplace_back(itQuestScript->second.GetCode());
				vcode_sizes.emplace_back(itQuestScript->second.GetSize());
			}
		}
	};
