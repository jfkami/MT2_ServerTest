...
class ITEM_MANAGER : public singleton<ITEM_MANAGER>
{
	public:
		...
		template <typename T>
		bool GetValidVnum(const char * str, T & outVnum) // @fixme303
		{
			try {
				std::size_t pos{};
				outVnum = std::stoi(str, &pos);
				if (strlen(str) != pos)
					return false;
				return GetTable(outVnum);
			} catch(...) {
				return false;
			}
		}
	...
	protected:
		...
		std::unordered_map<DWORD,TItemTable*> m_mapItemProto;
		#endif
